...
int allocate(int pid, int size) {
    ...
    if (base_address == -1) {
        // No suitable hole found
        printf("Error: Insufficient memory to allocate for Process %d.\n", pid);
        return 0;
    }
    ...
    printf("Process %d allocated successfully. Base Address: %d\n", pid, base_address);
...
int is_allowed_address(int pid, int addr) {
    ...
    printf("Error: Illegal memory operation for Process %d. Terminating the process.\n", pid);
    deallocate(pid);
    return 0;
}